    ...
    <div class="row row-cards row-deck">
        ...
        @foreach($items as $r)
            <div class="col-md-4 data_Table" id="data_Table{{$r->id}}">
                <div class="card">
                    <img class="card-img-topbr-tr-0 br-tl-0" src="{{$r->img()}}" alt="{{$r->name}}">
                    <div class="card-header">
                        <h5 class="card-title">{{$r->name}}</h5>
                    </div>
                    <div class="card-body">
                        <a class="btn btn-sm btn-primary" href="{{route('dashboard_posts.add_edit',['id' => $r->id])}}"><i class="fa fa-edit"></i> Edit</a>
                        <a class="btn btn-sm btn-danger btn_delete_current" data-id='{{$r->id}}' href="#"><i class="fa fa-trash"></i> Delete</a>
                        <div class="material-switch pull-right">
                            <input id="someSwitchOptionSuccess{{$r->id}}" class="btn_featured" data-id="{{$r->id}}" type="checkbox" {{$r->featured == 1 ? "checked" : ""}}>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.ajax_delete_all', function () {
                $.ajax({
                    url: "{{ route('dashboard_posts.deleted_all') }}",
                    method: "get",
                    data: {
                        "array": array,
                    },
