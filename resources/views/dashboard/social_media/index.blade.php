...
@section('create_btn'){{route('dashboard_social_media.add_edit')}}@endsection
    ...
    <div class="row row-cards row-deck">
        ...
        @foreach($items as $r)
            <div class="col-md-4 data_Table" id="data_Table{{$r->id}}">
                <div class="card">
                    <img class="card-img-topbr-tr-0 br-tl-0" src="{{path()}}sm.jpg" alt="{{$r->name}}">
                    <div class="card-header">
                        <h5 class="card-title">{{$r->name}}</h5>
                    </div>
                    <div class="card-body">
                        <a class="btn btn-sm btn-primary" href="{{route('dashboard_social_media.add_edit',['id' => $r->id])}}"><i class="fa fa-edit"></i> Edit</a>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn_featured', function () {
                ...
                $.ajax({
                    url: "{{ route('dashboard_social_media.featured') }}",
                    method: "get",
                    data: {
                        "id": id,
                    },
            ...
            $('.btn_deleted').on("click",function () {
                var id = $('#iddel').val();
                $.ajax({
                    url:"{{ route('dashboard_social_media.deleted') }}",
                    method:"get",
                    data : {
                        "id" : id,
                    },
            ...
            $(document).on('click', '.ajax_delete_all', function () {
                $.ajax({
                    url: "{{ route('dashboard_social_media.deleted_all') }}",
                    method: "get",
                    data: {
                        "array": array,
                    },
